...
const ExperienceTab = ({ title, bullets, dates, description, location }) => {
    ...
    const toggle = () => {
        if(!openTab.includes("[40vh]") || !openTab.includes("[100vh]")) {
            setOpenTab("w-[75%] h-[40vh] rounded-b-lg bg-lightwarm duration-300 max-md:h-[100vh]")
        } else {
    ...
    return (
        <>
            <div className="bg-lightblue drop-shadow-2xl w-[80%] h-[10vh] rounded-lg justify-between items-center flex mt-5 max">
                ...
                <button onClick={toggle} className={!openTab.includes("[40vh]") || !openTab.includes("[100vh]") ? "hover:scale-125 duration-300 text-darkred p-4" : "hover:scale-125 duration-300 text-darkred p-4 rotate-180 "}><FontAwesomeIcon className="text-4xl " icon={faChevronDown}/></button>
            </div>
            <div className={openTab}>
                <div className={!openTab.includes("[40vh]") || !openTab.includes("[100vh]") ? "opacity-0 duration-300 h-[0px]" : "w-full duration-300 h-full"}>
